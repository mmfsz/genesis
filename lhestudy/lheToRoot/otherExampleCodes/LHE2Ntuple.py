...
def build_TTree(data, outputname):
    ...
    m_tree.Branch("eventweight", m_eventweight, "eventweight/D")
    m_tree.Branch("scale", m_scale, "scale/D")
    m_tree.Branch("alpha_qed", m_qed, "alpha_qed/D")
    m_tree.Branch("alpha_qcd", m_qcd, "alpha_qcd/D")
...
if __name__ == "__main__":
    ...
    if pathlib.PurePosixPath(args.input).suffix != ".lhe" and pathlib.PurePosixPath(args.input).suffix != ".gz":
    ...
    if (pathlib.PurePosixPath(args.input).suffix == ".lhe" or pathlib.PurePosixPath(args.input).suffix == ".gz") and pathlib.PurePosixPath(args.output).suffix == ".root":
        if pathlib.PurePosixPath(args.input).suffix == ".gz":
            import gzip
            with gzip.open(args.input, 'rt') as f:
                root = read_xml(filename=f)
